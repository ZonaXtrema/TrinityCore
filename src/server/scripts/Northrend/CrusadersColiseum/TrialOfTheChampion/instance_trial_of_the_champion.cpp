            ...
            void SetData(uint32 type, uint32 data) override
            {
                switch (type)
                {
                    case DATA_REMOVE_VEHICLES:
                    {
                        ...
                        // Debemos eliminar los hechizos de defensa de los jugadores
                        Map::PlayerList const& players = instance->GetPlayers();
                        for (auto itr = players.begin(); itr != players.end(); ++itr)
                        {
                            Player* plr = itr->GetSource();
                            plr->RemoveAurasDueToSpell(62552); // Hechizo de defensa
                            plr->RemoveAurasDueToSpell(63130); // Escudo de nivel 1 (solo visual)
                            plr->RemoveAurasDueToSpell(63131); // Escudo de nivel 2 (solo visual)
                            plr->RemoveAurasDueToSpell(63132); // Escudo de nivel 3 (solo visual)
                        }
                        break;
                    }
